...
if(!isset($_POST['category']) || !isset($_POST['price']) || !isset($_POST['bookname']) || !isset($_POST['year']) || !isset($_POST['id']) || !isset($_POST['img'])){
    //header('Location: ..\adminud.php');
    exit();
}
$cat= $_POST['category'];
...
if(isset($_POST['discount'])) {
    mysqli_query($connection,"DELETE FROM `sale` WHERE `book_id`='$id'");
    $old_price=mysqli_query($connection, "SELECT `price` FROM `bookinfo` WHERE `id`='$id'");
    $old_price=$old_price->fetch_assoc();
    $old=$old_price['price'];
    $discount = $old_price['price']-$pr;
    if($discount>0) mysqli_query($connection,"INSERT INTO `sale` (`book_id`,`old_price`,`discount`) VALUES ('$id','$old','$discount')");
